...
library Uint256WithZeroMap {
    ...
    // AddressToUint256 Map

    struct AddressMap {
        mapping(address => uint256) _inner;
    }

    function set(AddressMap storage self, address key, uint256 value) internal {
        self._inner[key] = value + 1;
    }

    function get(AddressMap storage self, address key) internal view returns (uint256) {
        require(exists(self, key), "Uint256WithZeroMap: key does not exist");
        return self._inner[key] - 1;
    }

    function exists(AddressMap storage self, address key) internal view returns (bool) {
        return self._inner[key] != 0;
    }

    function remove(AddressMap storage self, address key) internal {
        delete self._inner[key];
    }
}
