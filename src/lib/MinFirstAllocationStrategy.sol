    ...
    function allocateToBestCandidate(
        ...
        for (uint256 j = 0; j < buckets.length; ++j) {
            if (buckets[j] >= capacities[j]) {
                ...
                buckets[j] > bestCandidateAllocation &&
                buckets[j] < allocationSizeUpperBound
            ) {
        ...
        allocated = Math.min(
            bestCandidatesCount > 1
                ? Math.ceilDiv(allocationSize, bestCandidatesCount)
                : allocationSize,
            Math.min(allocationSizeUpperBound, capacities[bestCandidateIndex]) -
                bestCandidateAllocation
        );
